...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable('simulations', (table) => {
    ...
    table.bigInteger('client_id').unsigned().references('clients.id').onDelete('CASCADE');
    table.bigInteger('vehicle_id').unsigned().references('vehicles.id').onDelete('CASCADE');
